...
hap_test <- as.data.frame(hap_test) #to get hap counts

#Permute the sex labels
hap_test$sex <- sample(hap_test$sex)

suppressMessages(hap_test2 <- hap_test %>% 
                   distinct(sample, sex) %>% 
...
#Output
write.table(haplotype_counts, paste0(opt$output, ".permuted.counts"), row.names = FALSE, sep="\t")
saveRDS(male_val, file = paste0(opt$output, ".malematrix.permuted.rds"))
saveRDS(female_val, file = paste0(opt$output, ".femalematrix.permuted.rds"))