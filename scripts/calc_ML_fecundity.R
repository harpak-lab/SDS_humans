...
haplotype_counts <- fread(opt$file, sep="\t", header = TRUE, 
  check.names=FALSE)
snps <- fread(opt$snpfile, sep="\t", header = TRUE, 
  check.names=FALSE)
...
ml_result <- lapply(winlist, 
  function(window) likelihood.out.f(haplotype_counts, window, pxhat = pxhat, shrinkage=FALSE))
...
ml_output1 <- merge(ml_output, snp_mapping, by.x = "Leading_SNP", 
  by.y = "Site1_SNP", all.x = TRUE)
ml_output1 <- merge(ml_output1 , snps, by.x = "Site0_SNP", 
  by.y = "SNP", all.x = TRUE, suffixes = c("", "_Site0"))
...
ml_output1 <- ml_output1[, c("Chrom", "Window", "Site1_Position", "Site1_SNP", 
  "Site1_MAF", "Site1_Allele0", "Site1_Allele1", "Site0_Position", "Site0_SNP", 
...
#Output
write.table(ml_output1, paste0(opt$output, ".f.maxl"), row.names = FALSE, sep="\t")
